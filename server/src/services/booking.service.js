  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      const data = {
        ...
        expiresAt: hold
          ? new Date(Date.now() + HOLD_SECONDS * 1000)
          : null,
      };
...
async function confirmBooking(appointmentId) {
  return prisma.$transaction(
    async (tx) => {
      ...
      // Lock slot again to serialize seat updates
      await tx.$queryRaw`
        SELECT id
        FROM "Slot"
...
// Background worker to expire pending holds
function startExpiryWorker() {
  ...
  setInterval(async () => {
    try {
      ...
      if (result > 0) {
        console.log(
          `[ExpiryWorker] Marked ${result} pending bookings as FAILED`
        );
      }
    } catch (e) {
